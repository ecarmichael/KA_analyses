function dwell_iv = KA_get_dwell(dwell_tbl, sess_id, min_dur)
...
% find the rat idx
this_rat = sub_code{contains(sub_code(:,2), sess_id(1:4)),1}; 
...
% remove dwells that are too short
dur = dwell_iv.tend - dwell_iv.tstart; 

rm_idx = dur<min_dur; 

dwell_iv.tstart(rm_idx) = []; 
dwell_iv.tend(rm_idx) = []; 
dwell_iv.usr(rm_idx) = []; 
