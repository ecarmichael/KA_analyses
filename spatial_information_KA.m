function [fr, occup, spatial_metrics] = spatial_information_KA(x,x_time, spk, trials, varargin)
...
% remove nans
spkcount(isnan(x)) = NaN; 
xbin(isnan(x)) = NaN; 

% spkcount, x and x_time from each Trial

spkcountT = cell(size(trials,1), length(spk));
xbinT = cell(size(trials,1),1);
for itrial=1:size(trials,1)
...
fr=NaN( size(trials,1) , length(bins)-1 , size(spkcount,1) );
occup=fr(:,:,1);
for itrial=1:size(trials,1)
    for ibin=(1:length(bins)-1)
        for icell=1:size(spkcount,1)
