...
if length(out.S.t{1}) / (out.pos.tvec(end) - out.pos.tvec(1)) < .1
...
subplot(2,3,3)
histogram(diff(out.S.t{1})*10000, 0:1:500);
xlabel('ISI ms')
ylabel('Spike count')
...
baseline_gau_mean = mean(S_gau_sdf);
baseline_gau_std = std(S_gau_sdf);

%% simple Z score and wilcoxon stats
turn_w = [-2 -1];
app_w = [-.5 .2]; 
rew_w = [0 1];
base_w = [-3 .5]; 
...
for ii = unique(rew_in)
    ...
    for jj = length(F_idx):-1:1
        base_S = restrict(out.S, rew_t(F_idx(jj)) + base_w(1), rew_t(F_idx(jj)) + base_w(2));
        
        if isempty(base_S.t{1})
            base_fr(jj) = 0;
        else
            base_fr(jj) = length(base_S.t{1})./ abs(diff(base_w));
        end
    end
    
    
    % turn
    this_S = restrict(out.S, rew_t(F_idx) + turn_w(1), rew_t(F_idx) + turn_w(2));
    out.turn_fr(ii) = (length(this_S.t{1})/length( F_idx))./ abs(diff(turn_w));
    out.turn_fr_z(ii) = (out.turn_fr(ii)- baseline_gau_mean) / baseline_gau_std;
    if abs(out.turn_fr_z(ii)) > 1.95
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at <strong>%s</strong> (vs baseline) <strong>(%0.2f SD) at %s arm </strong>\n',...
            mfilename, cell_to_process, 'turn (-2 - -1s)', out.turn_fr_z(ii), Zone_names{ii});
    end
    % same but wilcoxen
    trial_fr = []; 
    for jj = length(F_idx):-1:1
        trial_S = restrict(out.S, rew_t(F_idx(jj)) + turn_w(1), rew_t(F_idx(jj)) + turn_w(2));
        if isempty(trial_S.t{1})
            trial_fr(jj) = 0;
        else
            trial_fr(jj) = length(trial_S.t{1})./ abs(diff(turn_w));
        end
    end
    
    [out.turn_p, out.turn_h] = signrank(base_fr, trial_fr, 'tail', 'both', 'alpha', 0.05); 
    if out.turn_h
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at <strong>%s</strong> (vs baseline (-3 - -0.5s) <strong>(p = %0.5f) at %s arm </strong>\n',...
            mfilename, cell_to_process, 'turn (-2 - -1s)', out.turn_p , Zone_names{ii});
    end
    
    
    % approach
    this_S = restrict(out.S, rew_t(F_idx) + app_w(1), rew_t(F_idx) + app_w(2));
    out.app_fr(ii) = (length(this_S.t{1})/length( F_idx))./ abs(diff(app_w));
    out.app_fr_z(ii) = (out.app_fr(ii) - baseline_gau_mean) / baseline_gau_std;
    if abs(out.app_fr_z(ii)) > 1.95
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at <strong>%s</strong> (vs baseline) <strong>(%0.2f SD) at %s arm </strong>\n',...
            mfilename, cell_to_process, 'approach (-0.5 - -0.2s)', out.app_fr_z(ii) , Zone_names{ii});
    end
        % same but wilcoxen
    trial_fr = []; 
    for jj = length(F_idx):-1:1
        trial_S = restrict(out.S, rew_t(F_idx(jj)) + app_w(1), rew_t(F_idx(jj)) + app_w(2));
        if isempty(trial_S.t{1})
            trial_fr(jj) = 0;
        else
            trial_fr(jj) = length(trial_S.t{1})./ abs(diff(app_w));
        end
    end
    
    [out.app_p, out.app_h] = signrank(base_fr, trial_fr, 'tail', 'both', 'alpha', 0.05); 
    if out.app_h
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at <strong>%s</strong> (vs baseline (-3 - -0.5s) <strong>(p = %0.5f) at %s arm </strong>\n',...
            mfilename, cell_to_process, 'approach (-0.5 - -0.2s)', out.app_p , Zone_names{ii});
    end
    
    
    % reward
    this_S = restrict(out.S, rew_t(F_idx) + rew_w(1), rew_t(F_idx) + rew_w(2));
    out.rew_fr(ii) = (length(this_S.t{1})/length( F_idx))./ abs(diff(rew_w));
    out.rew_fr_z(ii) = (out.rew_fr(ii) - baseline_gau_mean) / baseline_gau_std;
    if abs(out.rew_fr_z(ii)) > 1.95
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at <strong>%s</strong> (vs baseline) <strong>(%0.2f SD) at %s arm </strong>\n',...
            mfilename, cell_to_process, 'reward (0-1s)', out.rew_fr_z(ii) , Zone_names{ii});
    end
    
        % same but wilcoxen
    trial_fr = []; 
    for jj = length(F_idx):-1:1
        trial_S = restrict(out.S, rew_t(F_idx(jj)) + rew_w(1), rew_t(F_idx(jj)) + rew_w(2));
        if isempty(trial_S.t{1})
            trial_fr(jj) = 0;
        else
            trial_fr(jj) = length(trial_S.t{1})./ abs(diff(rew_w));
        end
    end
    
    [out.rew_p, out.rew_h] = signrank(base_fr, trial_fr, 'tail', 'both', 'alpha', 0.05); 
    if out.rew_h
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at <strong>%s</strong> (vs baseline (-3 - -0.5s) <strong>(p = %0.5f) at %s arm </strong>\n',...
            mfilename, cell_to_process, 'reward (0-1s)', out.rew_p , Zone_names{ii});
    end
end


%% check for speed modulation as per Kropff et al. 2015

cfg_z.binsize = 0.03;
cfg_z.gauss_window = .25./cfg_z.binsize;
cfg_z.gauss_sd = 0.025./cfg_z.binsize;
...
tbin_edges = out.velo.tvec(1):cfg_z.binsize:out.velo.tvec(end);
...
FR_velo_int = interp1(tbin_edges(1:end-1), S_gau_sdf, out.velo.tvec); 

nan_idx = isnan(FR_velo_int); % points to exclude

move_idx = out.velo.data > 2; 


% get the pearson product
spd_corr  = corrcoef(FR_velo_int(~nan_idx & move_idx), out.velo.data(~nan_idx & move_idx)); 
out.spd_corr = spd_corr(1,2); 


% compare to shufflen
nShuf = 1000; 
Shuff_shifts = randperm(length(out.velo.data(~nan_idx)),nShuf); 
for ii = nShuf:-1:1
   this_shuff = corrcoef(FR_velo_int(~nan_idx & move_idx), circshift(out.velo.data(~nan_idx & move_idx), Shuff_shifts(ii))); 
...
if abs(out.spd_corr) > abs(out.spd_shuff_99_abs)
            ...
            mfilename, cell_to_process, out.spd_corr);
end
...
cfg_peth.window = [-2.5 2.5];
    cfg_peth.dt = 0.01; % fine resulition for regular PETH.
% cfg_peth.dt = cfg_z.binsize; % wider bins for 'Lap' plot later.
% cfg_peth.plot_type = 'shuff_zscore';
cfg_peth.plot_type = 'raw';
% cfg_peth.z_mean = gau_mean;
% cfg_peth.z_std = gau_std;
if exist([cell_to_process(1:end-4) '-wv.mat'], 'file')
    load([cell_to_process(1:end-4) '-wv.mat'])
    cfg_peth.waves.mWV = mWV;
    cfg_peth.waves.xrange = xrange;
...
for ii = unique(rew_in)
    ...
    All_trial.mean_S_gau{ii} = nanmean(All_trial.outputGau{ii},2);
    
    % re-define the 'pre' and 'post' means
    turn_idx = nearest_idx3([-2 -1], All_trial.outputIT{ii});
    app_idx = nearest_idx3([-0.5 0], All_trial.outputIT{ii});
    rew_idx = nearest_idx3([0 1], All_trial.outputIT{ii});
    exit_idx = nearest_idx3([1.5 2.5], All_trial.outputIT{ii});
    
        
    
    All_trial.turn{ii} =nanmean(All_trial.outputGau{ii}(turn_idx(1):turn_idx(2),:),1);
    All_trial.app{ii} =nanmean(All_trial.outputGau{ii}(app_idx(1):app_idx(2),:),1);
    All_trial.rew{ii} = nanmean(All_trial.outputGau{ii}(rew_idx(1):rew_idx(2),:),1);
    All_trial.exit{ii} = nanmean(All_trial.outputGau{ii}(exit_idx(1):exit_idx(2),:),1);
    
    All_trial.shuff_turn{ii} = nanmean(All_trial.shuffle{ii}(turn_idx(1):turn_idx(2),:),1);
    All_trial.shuff_app{ii} = nanmean(All_trial.shuffle{ii}(app_idx(1):app_idx(2),:),1);
    All_trial.shuff_rew{ii} = nanmean(All_trial.shuffle{ii}(rew_idx(1):rew_idx(2),:),1);
    All_trial.shuff_exit{ii} = nanmean(All_trial.shuffle{ii}(exit_idx(1):exit_idx(2),:),1);
    
    All_trial.shuff_turns_exit{ii} = All_trial.shuff_turn{ii} - All_trial.shuff_exit{ii};
    All_trial.shuff_rew_exit{ii} = All_trial.shuff_rew{ii} - All_trial.shuff_exit{ii};
    
    
    All_trial.turns_exit_z{ii} = ((mean(All_trial.turn{ii}) - mean(All_trial.exit{ii})) - nanmean(All_trial.shuff_turns_exit{ii}))./nanstd(All_trial.shuff_turns_exit{ii}, [], 2);
    All_trial.rew_exit_z{ii} =((mean(All_trial.rew{ii}) - mean(All_trial.exit{ii})) - - nanmean(All_trial.shuff_rew_exit{ii}))./nanstd(All_trial.shuff_rew_exit{ii}, [], 2);
    
    
    % test for baseline vs action
    %     [All_trial.H_app_mod{ii}, All_trial.p_app_mod{ii},~] = ttest2(All_trial.turn_means{ii} - All_trial.exit_means{ii},0,'Tail','both', 'alpha', 0.025); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
    %     [All_trial.H_pre_a{ii}, All_trial.b_pre_a{ii},~] = ttest2(All_trial.turn_means{ii} - All_trial.exit_means{ii},0,'Tail','right', 'alpha', 0.01); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
    %     [All_trial.H_app{ii}, All_trial.p_act{ii},~] = ttest2(All_trial.turn_means{ii} - All_trial.exit_means{ii},0,'Tail','left', 'alpha', 0.01); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
    
    if All_trial.turns_exit_z{ii} < -2.58 || All_trial.turns_exit_z{ii} > 2.58
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at action (vs exit) compared to shuffle <strong>(%0.2f SD) at %s arm </strong>\n', mfilename, cell_to_process, All_trial.turns_exit_z{ii}, Zone_names{ii});
    else
        fprintf('<strong>%s</strong> Cell:  %s is not significantly activity change at action (vs exit) compared to shuffle (%0.2f SD) at %s arm \n', mfilename, cell_to_process, All_trial.turns_exit_z{ii}, Zone_names{ii});
    end
    
    if All_trial.rew_exit_z{ii} < -2.58 || All_trial.rew_exit_z{ii} > 2.58
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at reward (vs exit) compared to shuffle <strong>(%0.2f SD) at %s arm </strong>\n', mfilename, cell_to_process, All_trial.rew_exit_z{ii}, Zone_names{ii});
    else
        fprintf('<strong>%s</strong> Cell:  %s is not significantly activity change at reward (vs exit) compared to shuffle (%0.2f SD) at %s arm \n', mfilename, cell_to_process, All_trial.rew_exit_z{ii}, Zone_names{ii});
    end
    
    % test for baseline vs action
    %     [All_trial.H_rew_mod{ii}, All_trial.p_rew_mod{ii},~] = ttest(All_trial.rew_means{ii} - All_trial.pre_stim_means{ii},0,'Tail','both', 'alpha', 0.025); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
    %     [All_trial.H_pre_r{ii}, All_trial.p_pre_r{ii},~] = ttest(All_trial.rew_means{ii} - All_trial.pre_stim_means{ii},0,'Tail','right', 'alpha', 0.05); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
    %     [All_trial.H_rew{ii}, All_trial.p_rew{ii},~] = ttest(All_trial.rew_means{ii} - All_trial.pre_stim_means{ii},0,'Tail','left', 'alpha', 0.05); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
    ...
    %     [All_trial.H_post{ii}, All_trial.p_post{ii},~] = ttest(All_trial.rew_means{ii} - All_trial.pre_stim_means{ii},0,'Tail','right', 'alpha', 0.01); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
    %
    %
    
    
    saveas(gcf, ['Zone_plots' filesep out.S.label{1}(1:strfind(out.S.label{1}, '.')-1) '_' Zone_names{ii} '_zcore_reward.png'])
    saveas(gcf, ['Zone_plots' filesep out.S.label{1}(1:strfind(out.S.label{1}, '.')-1)  '_' Zone_names{ii} '_zcore_reward.fig'])
    
    print(gcf,['Zone_plots' filesep out.S.label{1}(1:strfind(out.S.label{1}, '.')-1)  '_' Zone_names{ii} '_zcore_reward'],'-depsc')
...
[All_trial.outputS{5}, All_trial.outputIT{5}, All_trial.outputGau{5}, All_trial.shuffle{5}, All_trial.pre_stim_means{5}, All_trial.post_stim_means{5},] = SpikePETH_Shuff(cfg_peth, out.S, rew_t);
All_trial.mean_S_gau{5} = nanmean(All_trial.outputGau{5},2);

% re-define the 'pre' and 'post' means
turn_idx = nearest_idx3([-2 -1], All_trial.outputIT{5});
%     app_idx = nearest_idx3([-1 0], All_trial.outputIT{5});
rew_idx = nearest_idx3([0 1], All_trial.outputIT{5});
exit_idx = nearest_idx3([1.5 2.5], All_trial.outputIT{5});



All_trial.turn{5} =nanmean(All_trial.outputGau{5}(turn_idx(1):turn_idx(2),:),1);
All_trial.rew{5} = nanmean(All_trial.outputGau{5}(rew_idx(1):rew_idx(2),:),1);
All_trial.exit{5} = nanmean(All_trial.outputGau{5}(exit_idx(1):exit_idx(2),:),1);

All_trial.shuff_turn{5} = nanmean(All_trial.shuffle{5}(turn_idx(1):turn_idx(2),:),1);
All_trial.shuff_rew{5} = nanmean(All_trial.shuffle{5}(rew_idx(1):rew_idx(2),:),1);
All_trial.shuff_exit{5} = nanmean(All_trial.shuffle{5}(exit_idx(1):exit_idx(2),:),1);

All_trial.shuff_turns_exit{5} = All_trial.shuff_turn{5} - All_trial.shuff_exit{5};
All_trial.shuff_rew_exit{5} = All_trial.shuff_rew{5} - All_trial.shuff_exit{5};

    All_trial.turns_exit_z{5} = ((mean(All_trial.turn{5}) - mean(All_trial.exit{5})) - nanmean(All_trial.shuff_turns_exit{5}))./nanstd(All_trial.shuff_turns_exit{5}, [], 2);
    All_trial.rew_exit_z{5} =((mean(All_trial.rew{5}) - mean(All_trial.exit{5})) - - nanmean(All_trial.shuff_rew_exit{5}))./nanstd(All_trial.shuff_rew_exit{5}, [], 2);
    
    
    if All_trial.turns_exit_z{5} < -2.58 || All_trial.turns_exit_z{5} > 2.58
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at action (vs exit) compared to shuffle <strong>(%0.2f SD) across all arms </strong>\n', mfilename, cell_to_process, All_trial.turns_exit_z{5});
    else
        fprintf('<strong>%s</strong> Cell:  %s is not significantly activity change at action (vs exit) compared to shuffle (%0.2f SD) across all arms \n', mfilename, cell_to_process, All_trial.turns_exit_z{5});
    end
    
    if All_trial.rew_exit_z{5} < -2.58 || All_trial.rew_exit_z{5} > 2.58
        fprintf('<strong>%s</strong> Cell:  %s has significantly activity change at reward (vs exit) compared to shuffle <strong>(%0.2f SD) across all arms </strong>\n', mfilename, cell_to_process, All_trial.rew_exit_z{5});
    else
        fprintf('<strong>%s</strong> Cell:  %s is not significantly activity change at reward (vs exit) compared to shuffle (%0.2f SD) across all arms \n', mfilename, cell_to_process, All_trial.rew_exit_z{5});
    end
...
% end
%
% [All_trial.H_rew_mod{5}, All_trial.p_rew_mod{5},~] = ttest(All_trial.rew_means{5} - All_trial.pre_stim_means{5},0,'Tail','both', 'alpha', 0.025); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
% [All_trial.H_pre_r{5}, All_trial.p_pre_r{5},~] = ttest(All_trial.rew_means{5} - All_trial.pre_stim_means{5},0,'Tail','right', 'alpha', 0.01); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
% [All_trial.H_rew{5}, All_trial.p_rew{5},~] = ttest(All_trial.rew_means{5} - All_trial.pre_stim_means{5},0,'Tail','left', 'alpha', 0.01); % use a t-test to see if the distribution of post FR - pre FR across trials is greater than 0. Implying that the cell has significantly higher firing rate following reward delivery.
...
chil(text_idx(1)).Position = [chil(text_idx(1)).Position(1) max([All_trial.mean_S_gau{1}; All_trial.mean_S_gau{2}; All_trial.mean_S_gau{3}; All_trial.mean_S_gau{4}; All_trial.mean_S_gau{5}])*.3 chil(text_idx(1)).Position(3)];
chil(text_idx(2)).Position = [chil(text_idx(2)).Position(1) max([All_trial.mean_S_gau{1}; All_trial.mean_S_gau{2}; All_trial.mean_S_gau{3}; All_trial.mean_S_gau{4}; All_trial.mean_S_gau{5}])*.5 chil(text_idx(2)).Position(3)];
...
saveas(gcf, [out.S.label{1}(1:strfind(out.S.label{1}, '.')-1) '_all_zcore_reward.png'])
saveas(gcf, [out.S.label{1}(1:strfind(out.S.label{1}, '.')-1)  '_all_zcore_reward.fig'])
print(gcf,[out.S.label{1}(1:strfind(out.S.label{1}, '.')-1)  '_all_zcore_reward'],'-depsc')
...
saveas(gcf, ['Zone_plots' filesep out.S.label{1}(1:strfind(out.S.label{1}, '.')-1) '_rate_reward.png'])
saveas(gcf, ['Zone_plots' filesep out.S.label{1}(1:strfind(out.S.label{1}, '.')-1) '_rate_reward.fig'])
print(gcf,['Zone_plots' filesep out.S.label{1}(1:strfind(out.S.label{1}, '.')-1) '_rate_reward'],'-depsc')
...
saveas(gcf, ['Zone_plots' filesep out.S.label{1}(1:strfind(out.S.label{1}, '.')-1) '_reward_only_rate.png'])
saveas(gcf, ['Zone_plots' filesep out.S.label{1}(1:strfind(out.S.label{1}, '.')-1) '_reward_only_rate.fig'])
print(gcf,['Zone_plots' filesep out.S.label{1}(1:strfind(out.S.label{1}, '.')-1) '_reward_only_rate'],'-depsc')
