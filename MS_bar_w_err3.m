...
offsets_a = x_vals(1)+ sort((.1--.1).*rand(length(data_a),1) + -.1); %sort(MS_randn_range(length(data_a), 1, -.1, .1));
offsets_b = x_vals(2)+ sort((.1--.1).*rand(length(data_b),1) + -.1); %sort(MS_randn_range(length(data_b), 1, -.1, .1));
offsets_c = x_vals(3)+ sort((.1--.1).*rand(length(data_c),1) + -.1); %sort(MS_randn_range(length(data_c), 1, -.1, .1));
...
nan_mat = NaN(max([length(data_a), length(data_b), length(data_c)]), 3); 
nan_mat(1:length(data_a),1) = data_a; 
nan_mat(1:length(data_b),2) = data_b; 
nan_mat(1:length(data_c),3) = data_c; 

    switch stats
        case 'anova1'
            % disp('using ttest')
            if (length(data_a) ~= length(data_b)) & (length(data_b) ~= length(data_c))
                [p,stats_out.a_tbl, stats_out.stats] = anova1(nan_mat,[], 'off');
            else
...
if data_flag > 0 %&& length(data_a) == length(data_b)
        sc{1} = scatter( offsets_a, data_a,10, 'markerfacecolor', 'k', 'MarkerEdgeColor', 'none');
    sc{2} = scatter(offsets_b, data_b,15,  'markerfacecolor', 'k', 'MarkerEdgeColor', 'none');
        sc{3} = scatter(offsets_c, data_c,10,  'markerfacecolor', 'k', 'MarkerEdgeColor', 'none');
