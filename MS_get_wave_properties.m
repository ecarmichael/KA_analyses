...
if plot_flag % time consuming for fast spiking cells. so only compute with plots. 
    xbin_centers = -.25-0.01:0.01:0.25+0.01; % first and last bins are to be deleted later
    xbin_centers_short = -.02-0.001:0.001:0.02+0.001; % first and last bins are to be deleted later

    ac = zeros(size(xbin_centers));
    ac_short = zeros(size(xbin_centers_short));
    ...
    for iSpk = 1:length(S.t{1})
        ...
        ac_short = ac_short + hist(relative_spk_t,xbin_centers_short); % note that hist() puts all spikes outside the bin centers in the first and last bins! delete later.
    ...
    xbin_s = xbin_centers_short(2:end-1); % remove unwanted bins
    zero_idx_s = find(xbin_s == 0);
    ac_short = ac_short(2:end-1);
    ac_short(zero_idx_s) = 0;
end
...
%         plot(wave_tvec([rise_idx,return_idx]),[t_max,t_max]-abs(t_max*.2),"-b")
    ...
    text((wave_tvec(t_idx)-wave_tvec(p_idx))+wave_tvec(p_idx),p_max*1.1,['width = ' num2str(width*1000) 'ms'])
    title([strrep(S.label{1}(1:end-4), '_', ' ' )  '| Firing rate = ' num2str(spike_rate,2) 'Hz'])
